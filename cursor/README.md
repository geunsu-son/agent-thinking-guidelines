...
**적용 확인**:
- `Settings → Rules`에 4개 rule이 보이면 정상. 채팅에서 rule이 실제로 적용되면 응답 상단 컨텍스트 영역에 rule 이름이 표시된다.
...
```
[목적] 임원 보고용 기초 분석
[작업] @등록데이터.csv 에서 기수별 지역 분포 변화를 분석해줘
...
```
[목적] LMS 수업 유형 확장 대응
[작업] @schema.sql 기준으로 class_sub_type 레이어 추가 설계
[제약] 기존 class_type 값은 변경 불가
...
지정하지 않으면 rule의 기본 판단 기준(틀리면 재작업인가?)을 따른다.
