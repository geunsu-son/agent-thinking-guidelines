...
| 도구 | 확인 프롬프트 |
|---|---|
| Cursor | `이 프로젝트에 agent-thinking-guidelines(Cursor 버전)가 적용돼 있는지 확인해줘. .cursor/rules, skills, agents, memory, state를 점검하고 빠진 항목만 설치해.` |
| Claude Code | `이 프로젝트에 agent-thinking-guidelines(Claude Code 버전)가 적용돼 있는지 확인해줘. CLAUDE.md, .claude/skills, agents, memory, state를 점검하고 빠진 항목만 설치해.` |
...
2. 이 프로젝트의 CLAUDE.md, `.claude/skills`, `agents`, `memory`, `state`와 upstream을 비교해
3. 변경 요약을 먼저 보여줘
4. 반영 규칙:
...
2. **어긴 항목은 예시로 승격**: 에이전트가 반복해서 어기는 규칙은 나쁨/좋음 대비 예시로 추가한다. 규칙 서술보다 예시가 준수율을 가장 많이 올린다. 추가 위치: 항상 적용되는 원칙은 `docs/` + `core-principles.mdc`/`CLAUDE.md`, 상황 한정(분석·설계 등)은 해당 skill.
